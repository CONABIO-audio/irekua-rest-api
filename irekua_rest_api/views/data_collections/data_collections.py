...
class CollectionViewSet(utils.CustomViewSetMixin, ModelViewSet):
    ...
    serializer_mapping = utils.SerializerMapping.from_module(
        serializers.data_collections.data_collections
    ).extend(
    ...
    permission_mapping = utils.PermissionMapping(
        {
            ...
            "add_licence_type": [IsAuthenticated, IsAdmin],
            "add_metacollection": [IsAuthenticated, IsAdmin | IsDeveloper],
            "licences": [
                IsAuthenticated,
    ...
    def get_queryset(self):
        ...
        if self.action == "licences":
            collection_id = self.kwargs["pk"]
            return models.Licence.objects.filter(
                collection=collection_id
            )  # pylint: disable=E1101

        if self.action == "devices":
            collection_id = self.kwargs["pk"]
            return models.CollectionDevice.objects.filter(
                collection=collection_id
            )  # pylint: disable=E1101

        if self.action == "sites":
            collection_id = self.kwargs["pk"]
            return models.CollectionSite.objects.filter(
                collection=collection_id
            )  # pylint: disable=E1101

        if self.action == "users":
            collection_id = self.kwargs["pk"]
            return models.CollectionUser.objects.filter(
                collection=collection_id
            )  # pylint: disable=E1101

        if self.action == "sampling_events":
            collection_id = self.kwargs["pk"]
            return models.SamplingEvent.objects.filter(
                collection=collection_id
            )  # pylint: disable=E1101
