...
class CollectionTypeViewSet(
    ...
    serializer_mapping = utils.SerializerMapping.from_module(
        serializers.object_types.data_collections.types
    ).extend(
    ...
    def get_queryset(self):
        ...
        if self.action == "administrators":
            model = (
                models.CollectionType.administrators.through
        ...
        if self.action == "annotation_types":
            model = (
                models.CollectionType.annotation_types.through
            )  # pylint: disable=E1101
            collection_id = self.kwargs["pk"]
            return model.objects.filter(collectiontype_id=collection_id)
        ...
        if self.action == "sampling_event_types":
            model = (
                models.CollectionType.sampling_event_types.through
            )  # pylint: disable=E1101
            collection_id = self.kwargs["pk"]
            return model.objects.filter(collectiontype_id=collection_id)
