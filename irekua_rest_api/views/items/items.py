...
class ItemViewSet(
    ...
    permission_mapping = utils.PermissionMapping(
        {
            ...
            "add_secondary_item": [
                ...
                (
                    permissions.IsCreator
                    | IsSpecialUser  # TODO: Add correct permissions
                ),
            ],
        },
    ...
    def get_queryset(self):
        ...
        if self.action == "annotations":
            item_id = self.kwargs["pk"]
            return models.Annotation.objects.filter(
                item=item_id
            )  # pylint: disable=E1101

        if self.action == "secondary_items":
            item_id = self.kwargs["pk"]
            return models.SecondaryItem.objects.filter(
                item=item_id
            )  # pylint: disable=E1101
    ...
    def upload(self, request, pk=None):
        ...
        if item.item_file.name != "":
            return Response(
                "File previously uploaded", status=status.HTTP_403_FORBIDDEN
            )
    ...
    def download(self, request, pk=None):
        ...
        if item.item_file.name == "":
            return Response(
                "File not uploaded to server", status=status.HTTP_404_NOT_FOUND
            )
    ...
    def location(self, request, pk=None):
        item = self.get_object()
        serializer = serializers.sites.ItemLocationSerializer([item], many=True)
    ...
    def get_list_queryset(self):
        ...
        is_special_user = (
            user.is_superuser | user.is_curator | user.is_model | user.is_developer
        )
        if is_special_user:
    ...
    def get_normal_queryset(self, user):
        ...
        collections_with_permission = (
            models.CollectionUser.objects.filter(  # pylint: disable=E1101
                user=user.pk, role__in=perm.role_set.all()
            ).values("collection")
        )
        # TODO
